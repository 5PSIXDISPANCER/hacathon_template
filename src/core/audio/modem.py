...
class Modem:
    ...
    def encode_large_file(self,input_file_path: str, output_wav_path: str):
        ...
        return file_size
    # --- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЕКОДЕРА ---
    def bandpass_filter(self,samples, fs, lowcut, highcut, order=5):
        nyq = 0.5 * fs
        b, a = scipy.signal.butter(order, [lowcut / nyq, highcut / nyq], btype='band')
        return scipy.signal.filtfilt(b, a, samples)

    def soft_demodulate(self,samples, fs, sym_dur, K_channels, freqs):
        N = int(fs * sym_dur)
    ...
    def bytes_to_nibbles(self,bs):
        seq = []
        for byte in bs:
            seq.extend([byte >> 4, byte & 0x0F])
        return np.array(seq, dtype=int)

    def find_flag_soft(self,soft, flag_nibbles, K_channels):
        L = len(flag_nibbles)
    ...
    def decode_large_file_from_samples(self,samples: np.ndarray, output_file_path: str, total_file_size: int):
        ...
        sig = self.bandpass_filter(sig, FS, lowcut, highcut)

        freqs = np.array([[F_BASE + k*CHANNEL_SPACING + s*TONE_STEP for s in range(16)] for k in range(K)])
        soft = self.soft_demodulate(sig, FS, SYM_DUR, K, freqs)

        pre_nib = self.bytes_to_nibbles(PREAMBLE)
